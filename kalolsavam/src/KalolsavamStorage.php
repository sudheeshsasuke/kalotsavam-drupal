...
namespace Drupal\kalolsavam;

/**
 * Class DbtngExampleStorage.
 */
class KalolsavamStorage {
  ...
  public static function insert(array $entry) {
    $return_value = NULL;
    try {
      $return_value = db_insert('kalolsavam')
        ->fields($entry)
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public static function load(array $entry = []) {
    ...
    $select = db_select('kalolsavam', 'example');
    $select->fields('example')->where('uid = :uid', $arguments)->execute();
  ...
  public static function advancedLoad() {
    $select = db_select('kalolsavam', 'e');
    // Join the users table, so we can get the entry creator's username.
    $select->join('users_field_data', 'u', 'e.uid = u.uid');
    // Select these specific fields for the output.
    $select->addField('e', 'pid');
    $select->addField('u', 'name', 'username');
    $select->addField('e', 'competition');
    $select->addField('e', 'chest_no');
    $select->addField('e', 'mark');
  ...
  public static function aggregateMarkLoad() {
    $select = db_select('kalolsavam', 'e');
    // Select these specific fields for the output.
    $select->addField('e', 'AVG(mark)');
    // // Filter only persons named "John".
    $select->condition('e.competition', 'singing');
    // // Filter only persons older than 18 years.
    // $select->condition('e.age', 18, '>');
    // // Make sure we only get items 0-49, for scalability reasons.
    // $select->range(0, 50);
